    ...
    <form class="container my-5" @submit.prevent="submitBooking" v-if="route.name !== 'view-booking'">
        ...
        <div class="col-sm-10">
          <div class="form-check">
            <input class="form-check-input" type="radio" v-model="booking.Payment" id="gridRadios1"
                   value="Creadit Card" checked>
            <label class="form-check-label" for="gridRadios1">
              Pay with Creadit Card
            </label>
          </div>
          <div class="form-check">
      ...
      <div class="row mb-3">
        ...
        <select class="form-select" aria-label="Default select example" v-model="booking.superhero"
          ...
          <option v-for="(superhero,index) in superheroes" :key="index" :value="superhero"> {{ superhero }}</option>
        </select>
      </div>
      <div class="row mb-3">
      ...
      <button type="button" class="btn btn-danger" v-if="route.name === 'update-booking'" v-on:click="deleteBooking">
        Delete Booking
      </button>
    </form>
